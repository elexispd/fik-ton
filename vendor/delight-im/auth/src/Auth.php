...
final class Auth extends UserManager {
				...
				if (!empty($parts[0]) && !empty($parts[1])) {
					try {
						$rememberData = $this->db->selectRow(
							'SELECT a.user, a.token, a.expires, b.email, b.username, b.status, b.roles_mask, b.force_logout FROM ' . $this->makeTableName('users_remembered') . ' AS a JOIN ' . $this->makeTableName('mallusrs') . ' AS b ON a.user = b.mallUsrID WHERE a.selector = ?',
							[ $parts[0] ]
						);
					}
					catch (Error $e) {
			...
			if (($_SESSION[self::SESSION_FIELD_LAST_RESYNC] + $this->sessionResyncInterval) <= \time()) {
				...
				try {
					$authoritativeData = $this->db->selectRow(
						'SELECT email, username, status, roles_mask, force_logout FROM ' . $this->makeTableName('mallusrs') . ' WHERE id = ?',
						[ $this->getUserId() ]
					);
				}
				catch (Error $e) {
		...
		if ($this->isLoggedIn()) {
			...
			try {
				$expectedHash = $this->db->selectValue(
					'SELECT password FROM ' . $this->makeTableName('mallusrs') . ' WHERE id = ?',
					[ $this->getUserId() ]
				);
			}
			catch (Error $e) {
	...
	protected function onLoginSuccessful($userId, $email, $username, $status, $roles, $forceLogout, $remembered) {
		...
		try {
			$this->db->update(
				$this->makeTableNameComponents('mallusrs'),
				[ 'last_login' => \time() ],
				[ 'mallUsrID' => $userId ]
			);
		}
	...
	public function confirmEmail($selector, $token) {
		...
		try {
			$confirmationData = $this->db->selectRow(
				'SELECT a.mallUsrID, a.user_id, a.email AS new_email, a.token, a.expires, b.email AS old_email FROM ' . $this->makeTableName('users_confirmations') . ' AS a JOIN ' . $this->makeTableName('mallusrs') . ' AS b ON b.mallUsrID = a.user_id WHERE a.selector = ?',
				[ $selector ]
			);
		}
		catch (Error $e) {
				...
				if ($confirmationData['expires'] >= \time()) {
					...
					try {
						$this->db->update(
							...
							[
								...
								'mallUsrEmailStatus' => 1
							],
							[ 'mallUsrID' => $confirmationData['user_id'] ]
						);
					}
					...
					try {
						$this->db->delete(
							...
							[ 'mallUsrID' => $confirmationData['mallUsrID'] ]
						);
					}
					catch (Error $e) {
						throw new DatabaseError($e->getMessage());
		...
		if (!$this->isLoggedIn()) {
			if ($emailBeforeAndAfter[1] !== null) {
				...
				$userData = $this->getUserDataByEmailAddress(
					...
					[ 'mallUsrID', 'email', 'username', 'status', 'roles_mask', 'force_logout' ]
				);

				$this->onLoginSuccessful($userData['mallUsrID'], $userData['email'], $userData['username'], $userData['status'], $userData['roles_mask'], $userData['force_logout'], true);

				if ($rememberDuration !== null) {
					$this->createRememberDirective($userData['mallUsrID'], $rememberDuration);
				}
			}
		}
		...
		if ($this->isLoggedIn()) {
			...
			try {
				$existingUsersWithNewEmail = $this->db->selectValue(
					'SELECT COUNT(*) FROM ' . $this->makeTableName('mallusrs') . ' WHERE email = ?',
					[ $newEmail ]
				);
			}
			catch (Error $e) {
			...
			try {
				$verified = $this->db->selectValue(
					'SELECT verified FROM ' . $this->makeTableName('mallusrs') . ' WHERE id = ?',
					[ $this->getUserId() ]
				);
			}
			catch (Error $e) {
	...
	public function forgotPassword($email, callable $callback, $requestExpiresAfter = null, $maxOpenRequests = null) {
		...
		$openRequests = $this->throttling ? (int) $this->getOpenPasswordResetRequests($userData['mallUsrID']) : 0;

		if ($openRequests < $maxOpenRequests) {
			$this->throttle([ 'requestPasswordReset', $this->getIpAddress() ], 4, (60 * 60 * 24 * 7), 2);
			$this->throttle([ 'requestPasswordReset', 'user', $userData['mallUsrID'] ], 4, (60 * 60 * 24 * 7), 2);

			$this->createPasswordResetRequest($userData['mallUsrID'], $requestExpiresAfter, $callback,$email);
		}
		else {
			throw new TooManyRequestsException('', $requestExpiresAfter);
		}
	}
	public function forgotPasswordAdmin($email, callable $callback, $requestExpiresAfter = null, $maxOpenRequests = null) {
		$email = self::validateEmailAddress($email);
	...
	private function authenticateUserInternal($password, $email = null, $username = null, $rememberDuration = null, callable $onBeforeSuccess = null) {
		...
		$columnsToFetch = [ 'mallUsrID', 'email', 'password', 'mallUsrEmailStatus', 'username', 'status', 'roles_mask', 'force_logout' ];
		...
		if (\password_verify($password, $userData['password'])) {
			...
			if (\password_needs_rehash($userData['password'], \PASSWORD_DEFAULT)) {
				...
				$this->updatePasswordInternal($userData['mallUsrID'], $password);
			}

			if ((int) $userData['mallUsrEmailStatus'] === 1) {
				if (!isset($onBeforeSuccess) || (\is_callable($onBeforeSuccess) && $onBeforeSuccess($userData['mallUsrID']) === true)) {
					$this->onLoginSuccessful($userData['mallUsrID'], $userData['email'], $userData['username'], $userData['status'], $userData['roles_mask'], $userData['force_logout'], false);
	...
	private function getUserDataByEmailAddress($email, array $requestedColumns) {
		try {
			...
			$userData = $this->db->selectRow(
				'SELECT ' . $projection . ' FROM ' . $this->makeTableName('mallusrs') . ' WHERE mallUsrEmail = ?',
				[ $email ]
			);
		}
		catch (Error $e) {
	 ...
	 * @throws AuthError if an internal problem occurred (do *not* catch)
	 */
	private function createPasswordResetRequest($userId, $expiresAfter, callable $callback,$email=null) {
	...
	public function resetPassword($selector, $token, $newPassword) {
		...
		try {
			$resetData = $this->db->selectRow(
				'SELECT a.id, a.user, a.token, a.expires, b.mallUsrEmail FROM ' . $this->makeTableName('users_resets') . ' AS a JOIN ' . $this->makeTableName('mallusrs') . ' AS b ON b.mallUsrID = a.user WHERE a.selector = ?',
				[ $selector ]
			);
		}
		catch (Error $e) {
		...
		if (!empty($resetData)) {
			...
			if ((int) $resetData['user'] != "") {

				if (\password_verify($token, $resetData['token'])) {
					if ($resetData['expires'] >= \time()) {
						//Check if password already exists
						if ($security_ob->checkUserPasswordByIDPasswrd($resetData['user'],$newPassword)['status']!=1){
							$newPassword = self::validatePassword($newPassword);
						...
						try {
							...
							$security_ob->updateUserPasswordByIDPasswrd($resetData['user'],$newPassword);
						...
						return [
							'mallUsrID' => $resetData['user'],
							'email' => $resetData['email']
						];
						}
						else{
							throw new NewPasswordExistException();
						}
					}
					else {
						throw new TokenExpiredException();
	...
	public function resetPasswordAndSignIn($selector, $token, $newPassword, $rememberDuration = null) {
		...
		if (!$this->isLoggedIn()) {
			...
			$this->onLoginSuccessful($idAndEmail['mallUsrID'], $idAndEmail['email'], $userData['username'], $userData['status'], $userData['roles_mask'], $userData['force_logout'], true);

			if ($rememberDuration !== null) {
				$this->createRememberDirective($idAndEmail['mallUsrID'], $rememberDuration);
			}
		}
		...
		if ($this->isLoggedIn()) {
			try {
				$enabled = $this->db->selectValue(
					'SELECT resettable FROM ' . $this->makeTableName('mallusrs') . ' WHERE id = ?',
					[ $this->getUserId() ]
				);
